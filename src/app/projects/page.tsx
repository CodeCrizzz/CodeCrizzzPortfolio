    ...
    <main className="flex-1 w-full max-w-[1100px] mx-auto my-12 px-5 flex flex-col items-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-[1000px] justify-center">
        {projects.map((project, index) => (
          <Reveal key={index} className="w-full relative h-[250px] md:h-[300px] rounded-[20px] overflow-hidden bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 shadow-[0_10px_25px_rgba(0,0,0,0.05)] group transition-colors duration-300">
            <Image
              src={project.image}
              alt={project.title}
              fill
            ...
            <div className="absolute bottom-0 w-full bg-white/95 dark:bg-black/95 p-5 text-center z-10 transition-all duration-500 ease-in-out group-hover:opacity-0 group-hover:translate-y-5">
              <h3 className="m-0 text-[0.9rem] text-slate-800 dark:text-slate-200 font-black">{project.title}</h3>
            </div>

            <div className="absolute inset-0 bg-white/85 dark:bg-black/90 flex items-center justify-center p-8 opacity-0 z-20 transition-opacity duration-500 ease-in-out group-hover:opacity-100">
